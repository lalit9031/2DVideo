...
def main() -> None:
    ...
    manifests = []
    shots = [shot for shot in episode.get("shots", []) if not shot.get("broll")]
    ...
    for index, shot in enumerate(shots, start=1):
        ...
        def report_shot_progress(fraction: float) -> None:
            write_stage_progress(
                ...
                stage="rig_render",
                message=f"Rig render {index}/{total_shots}: {shot['shot_id']}",
            )

        result = render_shot(episode, shot, shot_path, fps=FPS, progress_callback=report_shot_progress)
